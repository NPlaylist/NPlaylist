...
namespace NPlaylist.Pls
{
    public class PlsDeserializer : IPlaylistDeserializer<PlsPlaylist>
    {
        ...
        private static readonly Regex s_entryGroupRegex = new Regex($@"File\d+=({locationPattern})\n?(?:Title\d+=({titlePattern})\n?)?(?:Length\d+=({lengthPattern})\n?)?");
        ...
        {
            playlist.Version = Regex.Match(input, versionPattern).Groups[1].Value;
        }
        ...
        private IEnumerable<PlsItem> ExtractItems(string input)
        {
            ...
            foreach (var line in Regex.Split(StringUtils.RemoveFileUnusedLines(input), @"\n\n"))
            {
        ...
        {
            var match = s_entryGroupRegex.Match(StringUtils.RemoveExtraNewLines(block));
            if (!match.Success)
            {
